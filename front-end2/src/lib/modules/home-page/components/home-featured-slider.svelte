<script lang="ts">
	...
	let title = $derived(currentTour?.tour_name?.[$locale] || currentTour?.tour_name?.en || 'Featured Tour')
	let price = $derived(currentTour?.tour_price?.pax2 || currentTour?.tour_price?.pax1 || 0)
	let duration = $derived(currentTour?.tour_duration?.[$locale] || currentTour?.tour_duration?.en || 'Full Day')
</script>

{#if hotTours.length > 0 && currentTour}
	<section class="relative border-b border-stone-200 bg-stone-950 text-stone-100 overflow-hidden">
			...
			{#key currentIndex}
				{#if currentTour.img_cover?.asset}
					<img
						...
						src={url_for(currentTour.img_cover).width(1600).height(900).auto('format').quality(80).url()}
						alt={currentTour.img_cover?.caption || title}
						class="h-full w-full object-cover opacity-35 filter brightness-90" />
				{/if}
			{/key}
			<div class="absolute inset-0 bg-gradient-to-r from-stone-950 via-stone-950/80 to-stone-950/40"></div>
		</div>
			...
			<div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-12">
				<div class="max-w-2xl">
					...
					<div class="mb-4 flex flex-wrap items-center gap-3">
						<span class="inline-flex items-center gap-1.5 bg-terracotta px-3 py-1 text-xs font-semibold uppercase tracking-widest text-white shadow-md">
							<svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="currentColor"><path d="M12 .587l3.668 7.431 8.2 1.192-5.934 5.784 1.399 8.169-7.333-3.856-7.333 3.856 1.399-8.169-5.934-5.784 8.2-1.192zm0 5.702l-2.232 4.522-4.991.725 3.612 3.521-.852 4.972 4.463-2.347 4.463 2.347-.852-4.972 3.612-3.521-4.991-.725z"/></svg>
							<span>{$locale === 'vn' ? 'Tour Nổi Bật / Bán Chạy' : $locale === 'fr' ? 'Coups de Cœur' : 'Featured & Best Seller'}</span>
						</span>
						{#if currentTour.tour_id}
							<span class="border border-stone-700 bg-stone-900/80 px-2.5 py-0.5 text-xs font-mono text-stone-300">
								{currentTour.tour_id}
							</span>
						{/if}
					...
					<!-- Tour Title -->
					<h2 class="mb-5 font-serif text-3xl font-normal leading-tight text-white sm:text-4xl lg:text-5xl">
						<button
							type="button"
							class="text-left font-serif text-white hover:text-stone-300 transition-colors"
					...
					{#if currentTour.tour_highlights?.length}
						<div class="mb-8 space-y-2">
							{#each currentTour.tour_highlights.slice(0, 3) as item}
								...
								{#if hlText}
									<div class="flex items-center gap-2.5 text-xs text-stone-300 sm:text-sm font-light">
					...
					{#if currentTour.tour_tags?.length}
						<div class="mb-8 flex flex-wrap gap-2">
							{#each currentTour.tour_tags as tag}
								...
								{#if tagName}
									<span class="border border-stone-700/60 bg-stone-900/60 px-2.5 py-0.5 text-[11px] font-medium tracking-wide text-stone-300 backdrop-blur-sm">
										#{tagName}
									</span>
								{/if}
							{/each}
					...
					<div class="flex flex-wrap items-center gap-4">
						<button
							...
							{$locale === 'vn' ? 'Xem Chi Tiết Tour' : $locale === 'fr' ? 'Voir les Détails' : 'View Details'}
						</button>
						<button
				...
				<div class="flex flex-col lg:items-end justify-between self-stretch pt-6 lg:pt-0 border-t lg:border-t-0 border-stone-800">
					<div class="lg:text-right">
						...
						<div class="mt-1 flex items-baseline gap-1 lg:justify-end">
							<span class="font-serif text-3xl sm:text-4xl text-white font-normal">
								{format_price(price, $locale)}
							</span>
							<span class="text-xs text-stone-400 font-light">/ pax</span>
						</div>
					</div>
								...
								{#each hotTours as _, idx}
									<button
										...
										class={`h-1.5 rounded-full transition-all ${
											currentIndex === idx ? 'w-6 bg-terracotta' : 'w-2 bg-stone-700 hover:bg-stone-500'
										}`}
