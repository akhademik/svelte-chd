...
## Corpus Check
- 255 files · ~121,679 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 852 nodes · 1434 edges · 88 communities (26 shown, 33 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 12 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a8ebfc27`
...
6. `Tour` - 13 edges
7. `sendMail()` - 10 edges
8. `resolveCanonicalCategory()` - 10 edges
9. `getTourSlug()` - 10 edges
...
Cohesion: 0.07
Nodes (40): App, Locals, Platform, handle(), initFormatters(), extractUrl(), getLangCookie(), getPathNameWithoutBase() (+32 more)

### Community 2 - "schemas/index.ts"
Cohesion: 0.07
Nodes (32): addThousandSeparator(), CNumberInput(), parseNumber(), COLOR_PALETTE, keywords, prepare(), prepare(), BASE_FIELDS (+24 more)
...
Cohesion: 0.10
Nodes (8): seoDescription, seoKeywords, seoOgImage, seoTitle, ./$types, ./$types, opacity, ./$types
...
Cohesion: 0.15
Nodes (8): bookingModal, BookingModalState, GeneralKeyString, Highlights, Locale_Array, Locale_String, Price, Tag
...
### Community 12 - "hero-image.service.ts"
Cohesion: 0.13
...
Cohesion: 0.10
Nodes (21): withKvSnapshot(), cachedFetch(), memoryCache, mapSanityToBlogPost(), mapSanityToBlogPosts(), ALL_BLOGS_QUERY, EXCHANGE_RATES_QUERY, EXTRACT_BLOG_FIELDS (+13 more)
...
Cohesion: 0.18
Nodes (13): DEFAULT_EXCHANGE_RATES, defaultRates, ExchangeRates, exchangeRatesStore, getAvatarInitials(), filterLocalizedItems(), getLocalizedField(), hasLocalizedTitle() (+5 more)
...
Cohesion: 0.08
Nodes (35): #each(), getMenuUrl(), isMenuActive(), MenuItem, menuItems, MenuLink, sanityClient, sanityConfig (+27 more)
...
- **Why does `keywords` connect `schemas/index.ts` to `back-end/package.json`?**
  _High betweenness centrality (0.012) - this node is a cross-community bridge._
- **Why does `Locales` connect `i18n-types.ts` to `tour.service.ts`, `format-data.ts`, `format-data.test.ts`, `i18n-svelte.ts`?**
  _High betweenness centrality (0.012) - this node is a cross-community bridge._
