...
## Corpus Check
- 290 files · ~125,547 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 906 nodes · 1570 edges · 88 communities (29 shown, 27 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 13 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `585a4a54`
...
### Community 18 - "blog-detail-modal.svelte"
Cohesion: 0.17
Nodes (5): blogModal, BlogModalState, tourModal, TourModalState, ./$types
...
Cohesion: 0.12
Nodes (10): #each(), getMenuUrl(), isMenuActive(), MenuItem, menuItems, MenuLink, isLocaleTransitioning, navAnimateHidden (+2 more)
...
Cohesion: 0.05
Nodes (56): DEFAULT_EXCHANGE_RATES, CACHE_POLICY, withKvSnapshot(), cachedFetch(), memoryCache, sanityClient, sanityConfig, mapSanityToBlogPost() (+48 more)

### Community 90 - "+layout.svelte"
Cohesion: 0.13
Nodes (5): bookingModal, BookingModalState, ./$types, opacity, ./$types

## Knowledge Gaps
- **255 isolated node(s):** `COLOR_PALETTE`, `BlockRenderProps`, `$schema`, `react-is`, `@types/styled-components` (+250 more)
...
- **Why does `Logger` connect `tour.service.ts` to `i18n-types.ts`, `rate-limiter.ts`?**
  _High betweenness centrality (0.025) - this node is a cross-community bridge._
- **Why does `sanity` connect `schemas/index.ts` to `back-end/package.json`, `type-others.ts`?**
  _High betweenness centrality (0.012) - this node is a cross-community bridge._
- **Why does `Locales` connect `i18n-types.ts` to `tour.service.ts`, `format-data.test.ts`, `i18n-svelte.ts`, `nav-bar-logic.ts`?**
